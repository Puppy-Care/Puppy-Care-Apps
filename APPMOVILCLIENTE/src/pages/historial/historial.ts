...
export class HistorialPage {
  ...
  historial() {
    ...
    // trae todas las encoimeindas 
    this._messageservice.getMessagesEnco(this._userService.getToken()).subscribe(response => {
      if (response.messagessEnco[0] != undefined) {
        this.vectorEncomiendas = response.messagessEnco;
        this.darvueltaencomienda();
        console.log(this.vectorEncomiendas);
        //localStorage.setItem("vectorEncomiendas", JSON.stringify(this.vectorEncomiendas));
      }
    }, (err) => { console.log("Existen COmplicaciones Intente mas tarde", err) }
    );
  ...
  irDetallesEnc(listado) {
    console.log('Este es mi listado ejejjeje ENC>>>', listado._id);

    this._messageservice.getMessagesEncoId(this._userService.getToken(), listado._id).subscribe(response => {

      console.log("loq ue vien por el socket mi pecs", response.messagess);
      localStorage.setItem("confirmacionEnc", JSON.stringify(response.messagess));

      localStorage.setItem('DirigeE', '1');
